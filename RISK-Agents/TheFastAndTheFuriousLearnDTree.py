...
###### pa2_learn_d_tree.py
#      Author: Chris Archibald, November 11, 2014
#      This file will learn a decision tree from a given datafile.
#      It is designed to work with data generated by the pa2_generate_data.py script, 
...
class DNode():
    ...
    def compute_entropy(self, p):
        ...
        #entrophy = -self.p * (math.log10(self.p) / math.log10(2)) - (1 - self.p) * (math.log10(1 - self.p) / math.log10(2))
    ...
    def determine_info_gain(self, feature_index):
        ...
        first_part = ((p0+n0) / (p0+n0+p1+n1)) * self.compute_entropy(x)

        second_part = ((p1+n1) / (p0+n0+p1+n1)) * self.compute_entropy(y)

        expected_entropy = float(first_part + second_part)

        info_gain = float(self.entropy - expected_entropy)
        #print('Info Gained: ', info_gain)
...
def print_usage():
    print 'USAGE: python pa2_learn_d_tree.py data_filename'
...
if __name__ == "__main__":
    ...
    savefilename = 'pa2_trees\\' + datafilename[9:-4] + '_' + str(depth) + '.dtree'
