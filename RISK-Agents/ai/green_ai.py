...
def getAction(state, time_left=None):
    ...
    if myDTree is None:
        print 'Loading DTree'
        myDTree = loadDTree('pa2_trees\pa2_10000_5.dtree')
        print 'Done.'
    ...
    if state.turn_type == 'Attack':
        ...
        for i in range(len(actions)):
            if (actions[i].to_territory == None and actions[i].from_territory == None and actions[i].troops == None):
                value = 0
                if (value > best_value):
                    best_value = value
                    best_action = i
                break
            value = state.armies[state.board.territory_to_id[actions[i].from_territory]] - state.armies[state.board.territory_to_id[actions[i].to_territory]]
            if (value <= 3):
                value = -9999
