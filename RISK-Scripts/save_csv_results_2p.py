...
def print_usage():
    print 'USAGE: python save_csv_results.py TOURNAMENT_RESULTS_FILE'


def save_line(line_agent, agents, winnings, of):
    of.write(line_agent + ',')
    for a in agents:
        if a == line_agent:
            of.write(',')
...
def save_winnings(agents, winnings, apm, filename):
    win_fname = filename[:-4] + '_crosstable ' + '.csv'
    ...
    for aaa in agents:
        save_line(aaa, agents, winnings, win_file)
        win_file.write('\n')
